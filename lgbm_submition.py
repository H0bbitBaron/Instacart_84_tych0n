...
if __name__ == '__main__':
    ...
    #                              dtype={'department_id': np.uint8, 'department': 'category'})
        order_prior = pd.read_csv(os.path.join(path, "src/order_products__prior.csv"), dtype={'order_id': np.uint32,
                                                                                          'product_id': np.uint16,
                                                                                          'add_to_cart_order': np.uint8,
                                                                                          'reordered': bool})
        order_train = pd.read_csv(os.path.join(path, "src/order_products__train.csv"), dtype={'order_id': np.uint32,
                                                                                          'product_id': np.uint16,
                                                                                          'add_to_cart_order': np.uint8,
                                                                                          'reordered': bool})
        orders = pd.read_csv(os.path.join(path, "src/orders.csv"), dtype={'order_id': np.uint32,
                                                                      'user_id': np.uint32,
                                                                      'eval_set': 'category',
        ...
        products = pd.read_csv(os.path.join(path, "src/products.csv"), dtype={'product_id': np.uint16,
                                                                          'aisle_id': np.uint8,
                                                                          'department_id': np.uint8})
    
        product_embeddings = pd.read_pickle('data/buffers/product_embeddings.pkl')
        product_embeddings = product_embeddings[embedings + ['product_id']]
    
        order_train = pd.read_pickle(os.path.join(path, 'buffers/chunk_0.pkl'))
        ...
        product_periods = pd.read_pickle(os.path.join(path, 'buffers/product_periods_stat.pkl')).fillna(9999)
        ...
        user_dep_stat = pd.read_pickle('data/buffers/user_department_products.pkl')
        user_aisle_stat = pd.read_pickle('data/buffers/user_aisle_products.pkl')
    ...
    features = [
        ...
        , 'up_mean_order', 'up_dueness', 'up_alt_dueness'
    ...
    not_included = [
            'reordered', 'days_since_ratio', 'product_reorder_rate', 'product_orders', 'UP_orders_ratio', 'up_median_cart_position'
            , 'user_total_items', 'UP_reorder_rate', 'user_average_days_between_orders', 'user_id', 'eval_set', 'product_reorders'
            , 'order_id', 'total_distinct_items', 'product_name', 'add_to_cart_order_relative_median', 'add_to_cart_order_inverted_median'
            , 'order_dow_median', 'order_hour_of_day_median', 'UP_orders', 'UP_average_pos_in_cart', 'user_total_orders', 'UP_orders_since_last'
            , 'user_median_days_since_prior', 'days_since_prior_order_median']
    data = order_train[features]
    labels = order_train[['reordered']].values.astype(np.float32).flatten()
    del order_train, not_included
    gc.collect()
    #
    ...
    #
    del features, order_test, categories, embedings
    gc.collect()
    ...
    del cat_features
    gc.collect()
    with open('models/lgbm/sh1ngPlus_magic_markov/hyperparams_iter.pkl', 'rb') as handle:
        params = pickle.load(handle)
    ### Averaging ###
    for i in range(7,8):
        params['feature_fraction_seed'] = randint(1, 100)
        params['bagging_seed '] = randint(1, 100)
        params['drop_seed'] = randint(1, 100)
        params['data_random_seed'] = randint(1, 100)
        with open('models/lgbm/sh1ngPlus_magic_markov/hyperparams_iter_{}.pkl'.format(i), 'wb') as handle:
        ...
        logloss = log_loss(labels, gbm.predict(data))
        gbm.save_model("models/lgbm/sh1ngPlus_magic_markov/gbmodel_{}_{}.txt".format(i, logloss))
        print('Logloss: ',logloss)
        prediction = gbm.predict(data_val)
        result = pd.DataFrame({'product_id': products, 'order_id': orders, 'prediction': prediction})
        result.to_pickle('data/predictions/prediction_lgbm_iter_{}.pkl'.format(i))
        del result
        gc.collect()
    del labels, data, data_val, products, orders
    gc.collect()
    # Merging predictions
    '''
    pred_x = pd.read_pickle('data/predictions/prediction_lgbm_iter_1.pkl')
    #2
    pred_y = pd.read_pickle('data/predictions/prediction_lgbm_iter_2.pkl')
    data_avg = pd.merge(pred_x, pred_y, on=['order_id','product_id'])
    del pred_x
    gc.collect()
    data_avg['prediction']=data_avg['prediction_x']+data_avg['prediction_y']
    data_avg.drop(['prediction_x','prediction_y',], axis=1, inplace=True)
    #3
    pred_y = pd.read_pickle('data/predictions/prediction_lgbm_iter_3.pkl')
    data_avg = pd.merge(data_avg, pred_y, on=['order_id','product_id'])
    data_avg['prediction']=data_avg['prediction_x']+data_avg['prediction_y']
    data_avg.drop(['prediction_x','prediction_y',], axis=1, inplace=True)
    #4
    pred_y = pd.read_pickle('data/predictions/prediction_lgbm_iter_4.pkl')
    data_avg = pd.merge(data_avg, pred_y, on=['order_id','product_id'])
    data_avg['prediction']=data_avg['prediction_x']+data_avg['prediction_y']
    data_avg.drop(['prediction_x','prediction_y',], axis=1, inplace=True)
    '''
    #5
    pred_x = pd.read_pickle('data/predictions/prediction_lgbm_iter_5.pkl')
    #6
    pred_y = pd.read_pickle('data/predictions/prediction_lgbm_iter_6.pkl')
    data_avg = pd.merge(pred_x, pred_y, on=['order_id','product_id'])
    data_avg['prediction']=data_avg['prediction_x']+data_avg['prediction_y']
    data_avg.drop(['prediction_x','prediction_y',], axis=1, inplace=True)
    #7
    pred_y = pd.read_pickle('data/predictions/prediction_lgbm_iter_7.pkl')
    data_avg = pd.merge(data_avg, pred_y, on=['order_id','product_id'])
    data_avg['prediction']=data_avg['prediction_x']+data_avg['prediction_y']
    data_avg.drop(['prediction_x','prediction_y',], axis=1, inplace=True)
    # Average
    data_avg['prediction']=data_avg['prediction']/3
    assert data_avg.shape[0] == 4833292
    data_avg.to_pickle('data/predictions/prediction_lgbm_iter_567.pkl')
